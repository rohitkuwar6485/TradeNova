...
exports.addInitialHoldings = async (req, res) => {
    try {
        const existing = await HoldingModel.find();
        if (existing.length > 0) {
            return res.status(400).send("Initial holdings already exist!");
        }
        ...
        res.send("Initial holdings inserted successfully!");
    } catch (error) {
        console.error(error);
